        ...
        for idx, item in tqdm.tqdm(enumerate(self.raw_dataset), total=len(self.raw_dataset),
            ...
            if not hide_gt:
                for kw in item["trunc_input_phrases"]:
                    ...
                    # STRICT ROUGE LOGIC (Repo Original)
...
class KeywordExtractorClf(pl.LightningModule):
    def __init__(self, model_config):
        ...
        self.clf = AutoModelForTokenClassification.from_pretrained(model_config['name'], config=config)
        
        # Enables gradient checkpointing to save VRAM.
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Use type_as to ensure weights are on correct device and dtype (BF16)
        loss_weight = torch.tensor([0.1, 1], device=self.device).type_as(logits)
