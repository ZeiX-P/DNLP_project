...
ZS_COT2_PROMPT_STR = {
    "local_mistral": ZS_COT2_PROMPT_STR_FOR_MISTRAL,
    "qwen": ZS_COT2_PROMPT_STR_FOR_QWEN,
    "llama": ZS_COT2_PROMPT_STR_FOR_LLAMA,
}
...
class CoT2Prompt(object):
    def __init__(
        ...
        self.prompt = customized_prompt or ZS_COT2_PROMPT_STR[model_name][dataset_name]
...
def get_prompt_fn(model_name, dataset, kw_strategy, kw_model_top_k, logits_threshold):
    ...
    elif kw_strategy == "cot_Nokey":
        return NaiveCoTPrompt(model_name, dataset)
    elif kw_strategy == "cot2":
        return CoT2Prompt(model_name, dataset)
    else:
...
def extract_final_summary(res):
    if "Final summary:" in res:
        return res.split("Final summary:", 1)[1].strip()
    elif "Final Summary:" in res:
        return res.split("Final Summary:", 1)[1].strip()
    else:
        return res.strip()
...
def run_inference(
    ...
    logits_threshold = estimate_logits_threshold(
        str(dataset_dir.joinpath("validation.jsonl")),
        75,
    )
    ...
    if kw_strategy in ["cot", "cot_Nokey", "cot2"]:
        summary = [extract_final_summary(res) for res in all_res]
        test_metrics = compute_rouge_score(inference_data, summary)
    else:
        test_metrics = compute_rouge_score(inference_data, all_res)

    metrics_file = pathlib.Path(output_dir).joinpath(f"{inference_on_split}_metrics.json")
...
def main():
    ...
    parser.add_argument(
        "--kw_strategy",
        choices=["disable", "sigext_topk", "cot", "cot_Nokey", "cot2"],
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="directory of train and validation data.",
    )
