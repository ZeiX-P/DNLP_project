...
class KeywordExtractorClf(pl.LightningModule):
    def __init__(self, model_config):
        ...
        if "num_random_blocks" in model_config and hasattr(config, "num_random_blocks"):
            ...
            logging.info(f"Set num_random_blocks={config.num_random_blocks}")

        if "block_size" in model_config and hasattr(config, "block_size"):
            config.block_size = model_config["block_size"]
            logging.info(f"Set block_size={config.block_size}")
...
def main():
    ...
    trainer.fit(
        KeywordExtractorClf(model_config),
        DataLoader(train_set, batch_size=1, shuffle=True, num_workers=0),
        DataLoader(val_set, batch_size=1, shuffle=False, num_workers=0)
    )
